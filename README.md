...
`03_monte_carlo_ensemble.py` generates .txt file for the ensemble members; this needs to be referred to in a bash shell script file, depending on your system, the full model simulation needs to be run on a parallel computing cluster.

For a simple proof of concept run, delete all but one .txt files in the "files" folder generated by `02_temp_conversion.py` before running `04_MAIN_script.py`. By default, `04_MAIN_script.py` is set to run with a test system of one ensemble member for input, and in this way works as demo on an ordinary pc and running all scripts in demo mode should take no longer than 20 minutes in total.
